...
contains
  ...
  subroutine fmin_brent(v,radius)
    ...
    print*,"MINIMIZE T"
    !
    call  brent_(solve_vca_t,v(1),[0.2d0,t_var+radius])   
    if((abs(t_var) .lt. 1.d-5) .or. (abs(t_var-v(1)-radius) .lt. 1.d-5) )then
      MULTIMAX=.true.
      print*,"MAXIMIZE T"
      call  brent_(solve_vca_t,v(1),[0.2d0,t_var+radius])
      if((abs(t_var) .lt. 1.d-5) .or. (abs(t_var-v(1)-radius) .lt. 1.d-5) )STOP "error on minimizing t"
    endif
    t_var=v(1)
    !
    print*,"MINIMIZE M"
    !
    MULTIMAX=.true.
    call  brent_(solve_vca_m,v(2),[0.2d0,M_var+radius])   
    if((abs(M_var) .lt. 1.d-5) .or. (abs(M_var-v(2)-radius) .lt. 1.d-5) )then
      MULTIMAX=.false.
      print*,"MAXIMIZE M"
      call  brent_(solve_vca_m,v(2),[0.2d0,M_var+radius])
      if((abs(M_var) .lt. 1.d-5) .or. (abs(M_var-v(2)-radius) .lt. 1.d-5) )STOP "error on minimizing t"
    endif
    M_var=v(2)
    !
    ...
    !
    MULTIMAX=.true.
    call  brent_(solve_vca_l,v(3),[0.2d0,lambda_var+radius])   
    if((abs(lambda_var) .lt. 1.d-5) .or. (abs(lambda_var-v(3)-radius) .lt. 1.d-5) )then
      MULTIMAX=.false.
      print*,"MAXIMIZE LAMBDA"
      call  brent_(solve_vca_l,v(3),[0.2d0,lambda_var+radius])
      if((abs(lambda_var) .lt. 1.d-5) .or. (abs(lambda_var-v(3)-radius) .lt. 1.d-5) )STOP "error on minimizing t"
    endif
    lambda_var=v(3)
    MULTIMAX=.false.

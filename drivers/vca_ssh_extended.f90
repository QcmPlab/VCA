program vca_ssh
  ...
  real(8)                                         :: vhop,whop,thop,energy_offset,wmixing
  logical                                         :: converged
  real(8)                                         :: wband
  !
  logical                                         :: wloop,wmin
  integer                                         :: nloop,iloop
  real(8)                                         :: vhop_var,whop_var,thop_var,dummy_omega
  ...
  call parse_input_variable(thop,"thop",finput,default=0.0d0,comment="second-nearest neighbor hopping")
  ...
  if((.not. wloop) .and. (.not. wmin))then
    dummy_omega = solve_vca(vhop,whop,thop)
  endif
  !
...
contains
  ...
  function solve_vca(vij,wij,tij) result(Omega)
    real(8)                      :: vij,wij,tij
    real(8)                      :: Omega
    !
    vhop_var=vij
   ...
   ! print*,"----- INIT -----"
    ...
    h_cluster = lso2nnn(Hloc_model(Nlso,vhop_var,whop_var,thop_var),Nlat,Nspin,Norb)
    call generate_hk()
    !print *, h_cluster
    call vca_solve(comm,h_cluster,h_k)
  ...
  function solve_vca1d(vij) result(Omega)
   ...
   function Hloc_model(N,vhop_,whop_,thop_) result (H0)
      integer                                               :: N,ispin,idimer
      real(8)                                               :: vhop_,whop_,thop_
      ...
      do ispin=1,Nspin
         do idimer=1,Ndimer
          ...
          if(2*idimer-1 +3 <= 2*Ndimer) then
            hopping_matrix(2*idimer-1,2*idimer-1 +3  ,ispin,ispin,:,:) = -thop_
            hopping_matrix(2*idimer-1 +3 ,2*idimer-1,ispin,ispin,:,:) = -thop_
          endif
          if(2*idimer+3 <= 2*Ndimer) then
            hopping_matrix(2*idimer,2*idimer +3  ,ispin,ispin,:,:) = -thop_
            hopping_matrix(2*idimer +3 ,2*idimer,ispin,ispin,:,:) = -thop_
          endif
          !
          ...
          !endif
          !if (Ndimer == 1) then
          !  hopping_matrix(1,2,ispin,ispin,:,:) = -whop_
          !  hopping_matrix(2,1,ispin,ispin,:,:) = -whop_
          !endif
         enddo
      enddo
      !
   ...
   function hk_model(kpoint,N) result(Hk)
      ...
      !
      do ispin=1,Nspin
            hopping_matrix(1,Nlat,ispin,ispin,:,:) = hopping_matrix(1,Nlat,ispin,ispin,:,:) + whop*exp(-xi*kpoint(1)*Ndimer)+ thop*exp(-2*xi*kpoint(1)*Ndimer)
            hopping_matrix(Nlat,1,ispin,ispin,:,:) = hopping_matrix(Nlat,1,ispin,ispin,:,:) + whop*exp( xi*kpoint(1)*Ndimer)+ thop*exp(-2*xi*kpoint(1)*Ndimer)
      enddo
      !
      Hk=nnn2lso(hopping_matrix,Nlat,Nspin,Norb)+hloc_model(N,vhop,whop,thop)
      !
